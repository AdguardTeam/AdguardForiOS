...
final class MigrationService: MigrationServiceProtocol {
    ...
    private func minorAndPatchMigration() {
        LOG.info("(MigrationService) - minorAndPatchMigration")
        ...
        if versionProvider.isLastVersionLessThan4_1 {
            LOG.info("(MigrationService) - reset the app to default state")
        ...
        if versionProvider.isMigrationFrom4_1To4_3Needed {
            LOG.info("(MigrationService) - app migration to 4.3 started")
            ...
            do {
                ...
                LOG.info("(MigrationService) - Successfully migrated old data to SDK")
            } catch {
                LOG.error("(MigrationService) - Failed to migrate old data to SDK; Error: \(error)")
            }
        }

        if versionProvider.isMigrationFrom4_3_0To4_3_1Needed {
            LOG.info("(MigrationService) - Starting migrate rules from 4.3.0 to 4.3.1")
            ...
            LOG.info("(MigrationService) - Successfully migrate safari rules from 4.3.0 to 4.3.1")

            dnsMigration4_3_1.migrate()
            LOG.info("(MigrationService) - Successfully migrate dns rules from 4.3.0 to 4.3.1")
        }
