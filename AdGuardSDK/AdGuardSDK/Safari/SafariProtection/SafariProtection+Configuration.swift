...
private let LOG = LoggerFactory.getLoggerWrapper("SafariProtection+Configuration")
...
extension SafariProtection {
    ...
    public func update(proStatus: Bool, onCbReloaded: ((_ error: Error?) -> Void)?) {
        workingQueue.sync {
            LOG.info("Updating proStatus from=\(self.configuration.proStatus) to=\(proStatus)")
            ...
            executeBlockAndReloadCbs {
                ...
                guard let self = self else {
                    LOG.error("update.proStatus.reloadContentBlockers; self is missing!")
                    DispatchQueue.main.async { onCbReloaded?(CommonError.missingSelf) }
                    return
                }

                if let error = error {
                    LOG.error("Error reloading CBs when updating proStatus; Error: \(error)")
                } else {
                    LOG.info("Successfully reloaded CBs after updating proStatus")
                }
    ...
    public func update(safariProtectionEnabled: Bool, onCbReloaded: ((_ error: Error?) -> Void)?) {
        workingQueue.sync {
            LOG.info("Updating safariProtection from=\(self.configuration.safariProtectionEnabled) to=\(safariProtectionEnabled)")
            ...
            executeBlockAndReloadCbs {
                ...
                guard let self = self else {
                    LOG.error("update.safariProtection.reloadContentBlockers; self is missing!")
                    DispatchQueue.main.async { onCbReloaded?(CommonError.missingSelf) }
                    return
                }

                if let error = error {
                    LOG.error("Error reloading CBs when updating safariProtection; Error: \(error)")
                } else {
                    LOG.info("Successfully reloaded CBs after updating safariProtection")
                }
                self.completionQueue.async { onCbReloaded?(error) }
            }
        }
    ...
    public func update(advancedProtectionEnabled: Bool) {
        workingQueue.sync {
            ...
            LOG.info("advancedProtection was updated = \(result)")
        }
    }
    ...
    public func update(advancedProtectionEnabled: Bool, onCbReloaded: ((Error?) -> Void)?) {
        workingQueue.sync {
            ...
            executeBlockAndReloadCbs {
                ...
                guard let self = self else {
                    LOG.error("update.advancedProtection.reloadContentBlockers; self is missing!")
                    DispatchQueue.main.async { onCbReloaded?(CommonError.missingSelf) }
                    return
                }

                if let error = error {
                    LOG.error("Error reloading CBs when updating advancedProtection; Error: \(error)")
                } else {
                    LOG.info("Successfully reloaded CBs after updating advancedProtection")
                }
                self.completionQueue.async { onCbReloaded?(error) }
            }
        }
    ...
    public func update(blocklistIsEnabled: Bool, onCbReloaded: ((_ error: Error?) -> Void)?) {
        workingQueue.sync {
            LOG.info("Updating blocklist state from=\(self.configuration.blocklistIsEnabled) to=\(blocklistIsEnabled)")
            ...
            executeBlockAndReloadCbs {
                ...
                guard let self = self else {
                    LOG.error("update.blocklistIsEnabled.reloadContentBlockers; self is missing!")
                    DispatchQueue.main.async { onCbReloaded?(CommonError.missingSelf) }
                    return
                }

                if let error = error {
                    LOG.error("Error reloading CBs when updating blocklist state; Error: \(error)")
                } else {
                    LOG.info("Successfully reloaded CBs after updating blocklist state")
                }
                self.completionQueue.async { onCbReloaded?(error) }
            }
        }
    ...
    public func update(allowlistIsEnabled: Bool, onCbReloaded: ((_ error: Error?) -> Void)?) {
        workingQueue.sync {
            LOG.info("Updating allowlist state from=\(self.configuration.allowlistIsEnabled) to=\(allowlistIsEnabled)")
            ...
            executeBlockAndReloadCbs {
                ...
                guard let self = self else {
                    LOG.error("update.allowlistIsEnabled.reloadContentBlockers; self is missing!")
                    DispatchQueue.main.async { onCbReloaded?(CommonError.missingSelf) }
                    return
                }

                if let error = error {
                    LOG.error("Error reloading CBs when updating allowlist state; Error: \(error)")
                } else {
                    LOG.info("Successfully reloaded CBs after updating allowlist state")
                }
                self.completionQueue.async { onCbReloaded?(error) }
            }
        }
    ...
    public func update(allowlistIsInverted: Bool, onCbReloaded: ((_ error: Error?) -> Void)?) {
        workingQueue.sync {
            LOG.info("Updating allowlist invertion state from=\(self.configuration.allowlistIsInverted) to=\(allowlistIsInverted)")
            ...
            executeBlockAndReloadCbs {
                ...
                guard let self = self else {
                    LOG.error("update.allowlistIsInverted.reloadContentBlockers; self is missing!")
                    DispatchQueue.main.async { onCbReloaded?(CommonError.missingSelf) }
                    return
                }

                if let error = error {
                    LOG.error("Error reloading CBs when updating allowlist invertion; Error: \(error)")
                } else {
                    LOG.info("Successfully reloaded CBs after updating allowlist invertion")
                }
                self.completionQueue.async { onCbReloaded?(error) }
            }
        }
    ...
    private func updateInternal(advancedProtectionEnabled: Bool) -> Bool {
        if configuration.advancedBlockingIsEnabled != advancedProtectionEnabled {
            ...
            LOG.info("Advanced protection hasn't been updated; reason - old value is equal to new value")
            return false
        }
    }
}
